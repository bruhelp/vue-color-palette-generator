  ...
  <div class="panel">
    ...
    <div class="row">
      ...
      <select v-model="localType">
        ...
        <!-- Добавляем новые опции по настроению -->
        ...
        <option value="nature">Природные</option>
      </select>
      
      <!-- Добавим небольшие описания для настроений -->
      <div v-if="showMoodDescription" class="mood-description">
        <small>{{ moodDescriptions[localType] }}</small>
      </div>
    </div>
    ...
    <div class="row">
      <div class="row-actions">
        <button class="btn primary" @click="onRandomClick">Случайный цвет</button>
...
export default {
  ...
  setup(props, { emit }){
    ...
    // Описания для настроений
    const moodDescriptions = {
      calm: 'Приглушённые, пастельные тона для расслабления',
      energetic: 'Яркие, насыщенные цвета для энергии',
      professional: 'Сдержанные тона для делового стиля',
      romantic: 'Нежные, розовые оттенки',
      nature: 'Земляные и зелёные природные тона'
    }
    
    // Показывать ли описание - используем computed
    const showMoodDescription = computed(() => {
      return ['calm', 'energetic', 'professional', 'romantic', 'nature'].includes(localType.value);
    })
